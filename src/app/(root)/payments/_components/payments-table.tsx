...
export function PaymentsTable({
  ...
  const columns: DataTableColumn<PaymentRow>[] = [
    ...
    },
    {
      id: "expected",
      header: "Expected",
      cell: (payment) => formatCurrency(payment.expectedAmount, currency),
    },
    {
      id: "received",
      header: "Received",
      cell: (payment) => formatCurrency(payment.receivedAmount, currency),
    },
    {
      id: "pending",
      header: "Pending",
      cell: (payment) => formatCurrency(payment.remainingAmount, currency),
    },
    {
      id: "status",
      header: "Status",
    ...
    },
    {
      ...
      cell: (payment) =>
        payment.transactions[0]
          ? format(new Date(payment.transactions[0].date), "MMM d, yyyy")
          : "-",
    },
    {
      id: "actions",
      ...
      cell: (payment) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onSelect={() => onAddPayment(payment.studentId)}>
              <WalletCards className="mr-2 h-4 w-4" />
              Add payment
            </DropdownMenuItem>
            <DropdownMenuItem onSelect={() => onViewHistory(payment.studentId)}>
              View history
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
  ...
  if (isLoading) {
    return (
      <DataTableState
        ...
        className="h-32 justify-center border border-solid border-pink-100 bg-white"
      />
    );
  }
  ...
  if (payments.length === 0) {
    return (
      <DataTableState
        ...
        description="Try another month or select a specific student."
        icon="💸"
      />
    );
  }
  ...
  return (
    <>
      <DataTable
        ...
        itemRowClassName="hover:bg-pink-50"
      />
